...
function Drawlevel($locs) {
	...
	foreach(Arrange($locs) as $loc){
		$locnum++;
		if( !(isset($xloc[$loc]) and isset($yloc[$loc])) ){
			$phi  = $cro * M_PI/180 + 2 * $locnum * M_PI / $nloc;
			$locw = pow($ncl,$lnw/10);
			$xloc[$loc] = intval((intval($xm/2) + $xo) + $csi * cos($phi) * $ctyscalx / $ctywght);
			$xloc[$loc] = intval((intval($ym/2) + $yo) + $csi * sin($phi) * $ctyscaly / $ctywght);
		}
		if($lev == "c" or $lev == "C"){
			$mapitems .= Drawitem($xct[$cty],$yct[$cty],$cdev[$cty],$l[1],$lev);
			$area = ($xct[$cty]-20) .",". ($yct[$cty]-20) .",". ($xct[$cty]+20) .",". ($yct[$cty]+20);
			$imgmap .= "<area href=?flt=". rawurlencode($cty) ."&lev=b&loi=1&draw=1 coords=\"$area\" shape=rect title=\"Show\">\n";
		}else{	
		}
	}
}
...
function Map() {
	...
	if($lev == "c" or $lev == "C"){									# Draw links after we set all coordinates
