...
if ($ina){
	...
	if($res){
		...
		while( ($n = @DbFetchRow($res)) ){
			...
			if(in_array("device",$col))	{echo "<td><a href=$_SERVER[PHP_SELF]?ina=device&opa==&sta=$ud&ord=ifname>$n[6]</a> <a href=$_SERVER[PHP_SELF]?ina=device&opa==&inb=ifname&opb==&sta=$ud&cop=AND&stb=$if>$n[7]</a></td>";}
